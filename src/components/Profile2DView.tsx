...
import { DirectivityCalculator } from '../acoustic/analysis/DirectivityIndex';
import { MultiPolarPlot } from '../acoustic/visualization/PolarPlot';
import { MultiFrequencyPlot } from '../acoustic/visualization/FrequencyPlot';
...
export default function Profile2DView({ hornParams, onToggle3D }: Profile2DViewProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [showAcousticView, setShowAcousticView] = useState<'profile' | 'dispersion' | 'frequency'>('profile');
  ...
  // Calculate acoustic analysis when enabled
  const acousticData = useMemo(() => {
    // Always calculate acoustic data for visualization
    const targetFreq = hornParams.targetFrequency || 1000;
    const mouthHeight = hornParams.mouthHeight || hornParams.mouthWidth;

    // Calculate dispersion
    const dispersionResult = DispersionAnalyzer.calculateRequiredMouthSize({
      horizontalAngle: hornParams.targetHorizontalDispersion || 60,
      verticalAngle: hornParams.targetVerticalDispersion || 40,
      frequency: targetFreq,
      mouthWidth: hornParams.mouthWidth,
      mouthHeight: mouthHeight
    });

    // Calculate frequency response
    const frequencyResponse = FrequencyResponseAnalyzer.calculateResponse(
      profilePoints,
      hornParams
    );

    // Calculate directivity
    const directivity = DirectivityCalculator.calculateFromCoverage(
      dispersionResult.horizontalBeamwidth,
      dispersionResult.verticalBeamwidth
    );

    // Generate polar patterns
    const horizontalPattern = DispersionAnalyzer.calculateDispersionPattern(
      profilePoints,
      targetFreq,
      'horizontal',
    ...
    const verticalPattern = DispersionAnalyzer.calculateDispersionPattern(
      profilePoints,
      targetFreq,
      'vertical',
      mouthHeight
    ...
    <div className="flex-1 flex flex-col gradient-bg relative">
      ...
      <div className="absolute bottom-4 left-4 z-10 flex flex-col gap-2">
        ...
        <div className="flex gap-2">
            <button
              onClick={() => setShowAcousticView('profile')}
              className={`glass-button px-4 py-2 rounded-lg text-white font-medium transition-all ${
                showAcousticView === 'profile' ? 'bg-white/20' : 'hover:bg-white/10'
              }`}
            >
              Profile
            </button>
            <button
              onClick={() => setShowAcousticView('dispersion')}
              className={`glass-button px-4 py-2 rounded-lg text-white font-medium transition-all ${
                showAcousticView === 'dispersion' ? 'bg-white/20' : 'hover:bg-white/10'
              }`}
            >
              Dispersion
            </button>
            <button
              onClick={() => setShowAcousticView('frequency')}
              className={`glass-button px-4 py-2 rounded-lg text-white font-medium transition-all ${
                showAcousticView === 'frequency' ? 'bg-white/20' : 'hover:bg-white/10'
              }`}
            >
              Response
            </button>
      ...
      {acousticData && (
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          {showAcousticView === 'dispersion' && (
            <div className="pointer-events-auto">
              <MultiPolarPlot
                horizontalData={acousticData.horizontalPattern}
                verticalData={acousticData.verticalPattern}
                width={800}
                height={400}
                title={`Dispersion Pattern at ${hornParams.targetFrequency || 1000} Hz`}
              />
            </div>
          )}
          {showAcousticView === 'frequency' && (
            <div className="pointer-events-auto">
              <MultiFrequencyPlot
                responseData={acousticData.frequencyResponse}
                width={800}
                height={600}
              />
            </div>
      ...
      {/* Acoustic Info Panel */}
      {acousticData && showAcousticView === 'profile' && (
        <div className="absolute bottom-4 right-4 glass-dark rounded-xl p-4 max-w-xs">
          <h3 className="text-sm font-bold text-white mb-2">Acoustic Analysis</h3>
          <div className="space-y-2 text-xs text-gray-300">
            <div>
              <span className="text-gray-400">Cutoff Frequency:</span>
              <span className="ml-2 text-white">{(hornParams.calculatedCutoffFrequency || acousticData.frequencyResponse.cutoffFrequency).toFixed(0)} Hz</span>
            </div>
            <div>
              <span className="text-gray-400">Directivity Index:</span>
              <span className="ml-2 text-white">{acousticData.directivity.directivityIndex.toFixed(1)} dB</span>
            </div>
            <div>
              <span className="text-gray-400">H-Beamwidth:</span>
              <span className="ml-2 text-white">{(hornParams.calculatedHorizontalDispersion || acousticData.dispersion.horizontalBeamwidth).toFixed(0)}°</span>
            </div>
            <div>
              <span className="text-gray-400">V-Beamwidth:</span>
              <span className="ml-2 text-white">{(hornParams.calculatedVerticalDispersion || acousticData.dispersion.verticalBeamwidth).toFixed(0)}°</span>
            </div>
            <div className="mt-2 pt-2 border-t border-white/10">
              <span className="text-gray-400">Required Mouth Size:</span>
              <div className="ml-2 text-white">
                {acousticData.dispersion.requiredWidth.toFixed(0)} × {acousticData.dispersion.requiredHeight.toFixed(0)} mm
              </div>
            </div>
          </div>
        </div>
